...
export type AddressCreateInput = Omit<Address, 'addressID'> & {
  isDefault?: true
}
...
type Activity = (ReviewComment | Question | Answer) & {
  type: string
  userEmail: string
}

export type Feed = Activity[]
export type UserFeed = ((ReviewComment | Review | Answer) & {
  type: string
})[]
...
export type UserSignupInput = Pick<
  User,
  'email' | 'password' | 'name'
>
...
export type UserUpdateInput = Partial<
  Pick<User, 'name' | 'email' | 'avatar' | 'cover'>
> & { info?: string | null }
...
export type UserAddressCreateInput = Partial<
  Pick<UserAddress, 'isDefault'>
>
...
export type ProductUpdateInput = Omit<
  ProductCreateInput,
  'listPrice'
> & { groupID: number; listPrice?: number }
...
export type ProductPublicData = Omit<
  ProductData,
  'createdAt' | 'updatedAt' | 'userID'
>

export type ReviewCreateInput = Pick<
  Review,
  'title' | 'content' | 'productID' | 'stars'
> &
...
export type ReviewUpdateInput = Partial<
  Pick<
    Review,
    'title' | 'content' | 'stars' | 'isVerified' | 'moderationStatus'
  >
>

export type ReviewCommentCreateInput = Pick<
  ReviewComment,
  'content' | 'parentReviewCommentID'
>

export type ReviewCommentUpdateInput = Partial<
  Pick<ReviewComment, 'content' | 'moderationStatus'>
>

export type QuestionCreateInput = Pick<Question, 'content'>

export type QuestionUpdateInput = Partial<
  Pick<Question, 'content' | 'moderationStatus'>
>

export type AnswerCreateInput = Pick<Answer, 'content'>

export type AnswerUpdateInput = Partial<
  Pick<Answer, 'content' | 'moderationStatus'>
>
...
export type ProductParametersInput = Pick<
  ProductParameter,
  'value' | 'parameterID'
>[]
...
export type OrderCreateInput = Pick<Order, 'shippingMethod'> & {
  addressID: number
  details: string
  paymentMethod: string
  promo?: string
...
export type OrderUpdateInput = Partial<
  Pick<Order, 'orderStatus' | 'shippingMethod'> & {
    addressID: number
  }
>
...
export type OrderProductInput = Omit<
  OrderProduct,
  'orderID' | 'productID'
>
...
export type InvoiceCreateInput = Pick<
  Invoice,
  'amount' | 'details' | 'orderID' | 'userID' | 'paymentMethod'
>

export type InvoiceUpdateInput = Partial<
  Pick<
    Invoice,
    'amount' | 'details' | 'invoiceStatus' | 'paymentMethod'
  >
>
...
export type OrderProductFullData = Pick<Product, 'title'> &
  Pick<
    OrderProduct,
    'productID' | 'orderID' | 'price' | 'qty' | 'size'
  > & { imageID: number }
