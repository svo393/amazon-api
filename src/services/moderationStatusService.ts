...
const addModerationStatus = async (
  moderationStatusInput: ModerationStatus
): Promise<ModerationStatus> => {
  const {
    rows: [addedModerationStatus]
  }: { rows: ModerationStatus[] } = await db.raw(
    `
    ? ON CONFLICT
      DO NOTHING
      RETURNING *;
  ...
  if (addedModerationStatus === undefined) {
    throw new StatusError(
      409,
      `ModerationStatus with name "${moderationStatusInput.moderationStatusName}" already exists`
    )
  }
...
const updateModerationStatus = async (
  ...
  const [updatedModerationStatus]: ModerationStatus[] = await db(
    'moderationStatuses'
  )
    .update(moderationStatusInput, ['*'])
    .where('moderationStatusName', req.params.moderationStatusName)

  if (updatedModerationStatus === undefined)
    throw new StatusError(404, 'Not Found')
  return updatedModerationStatus
}

const deleteModerationStatus = async (
  req: Request
): Promise<void> => {
