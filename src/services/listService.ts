  ...
  return await dbTrans(async (trx: Knex.Transaction) => {
    ...
    if (addedList === undefined) {
      throw new StatusError(
        409,
        `List with name "${listInput.name}" already exists`
      )
    }

    const {
      rows: [addedLP]
    }: { rows: ListProduct[] } = await trx.raw(
      `
      ? ON CONFLICT
        DO NOTHING
        RETURNING *;
...
const getListsByUser = async (req: Request): Promise<List[]> => {
  return await db('lists').where('userID', req.params.userID)
}
...
const getListByID = async (
  req: Request
): Promise<List & { products: ListProductData[] }> => {
  ...
  const listProducts: Pick<
    Product,
    'productID' | 'price' | 'isAvailable' | 'stock'
  >[] = await db('listProducts as lp')
    .select('lp.productID', 'p.price', 'p.isAvailable', 'p.stock')
