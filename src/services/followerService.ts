...
const getFollowersByUser = async (
  ...
  return await db('followers').where('follows', req.params.userID)
}

const getFollowedByUser = async (
  cursorInput: Pick<CursorInput, 'startCursor'> & { limit?: number },
  req: Request
): Promise<{
  userID: number
  follows: BatchWithCursor<Pick<User, 'userID' | 'avatar' | 'name'>>
}> => {
  const { startCursor, limit } = cursorInput
  const { userID } = req.params

  let users: Pick<User, 'userID' | 'avatar' | 'name'>[] = await db(
    'followers as f'
  )
    .select('u.userID', 'u.avatar', 'f.follows', 'u.name')
