...
const addVendor = async (
  ...
  if (addedVendor === undefined) {
    throw new StatusError(
      409,
      `Vendor with name "${vendorInput.name}" already exists`
    )
  }
  return addedVendor
}
...
const getVendors = async (
  vendorsFiltersinput: VendorsFiltersInput
): Promise<BatchWithCursor<VendorListData>> => {
  const { page = 1, sortBy = 'groupID', q } = vendorsFiltersinput

  const rawVendors: VendorListRawData[] = await db<Vendor>(
    'vendors as v'
  )
...
const getVendorByID = async (
  req: Request
): Promise<VendorListData> => {
  const rawVendor: VendorListRawData = await db<Vendor>(
    'vendors as v'
  )
