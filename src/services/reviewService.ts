...
const addReview = async (
  reviewInput: ReviewCreateInput,
  req: Request
): Promise<Pick<Review, 'reviewID'>> => {
  const now = new Date()

  const {
    rows: [addedReview]
  }: { rows: Review[] } = await db.raw(
    `
    ? ON CONFLICT
      DO NOTHING
      RETURNING *;
...
const getReviews = async (
  reviewsFiltersInput: ReviewsFiltersInput,
  req: Request
...
> & { groupID?: number; images?: Image[] }> => {
  ...
  const userHasPermission = ['ROOT', 'ADMIN'].includes(
    req.session?.role
  )
  ...
  let reviews: any[] = await db('reviews as r')
    ...
    .groupBy('r.reviewID', 'u.avatar', 'u.name', 'u.email')
  ...
  if (userHasPermission && userEmail !== undefined) {
    reviews = reviews.filter((r) =>
      r.userEmail?.toLowerCase().includes(userEmail.toLowerCase())
    )
  }

  if (moderationStatuses !== undefined) {
    reviews = reviews.filter((r) =>
      moderationStatuses.split(',').includes(r.moderationStatus)
    )
  }

  if (isVerified !== undefined) {
    reviews = reviews.filter((r) => r.isVerified === isVerified)
  }
  ...
  if (starsMin !== undefined) {
    reviews = reviews.filter((r) => r.stars >= starsMin)
  }

  if (starsMax !== undefined) {
    reviews = reviews.filter((r) => r.stars <= starsMax)
  }

  if (votesMin !== undefined) {
    reviews = reviews.filter((r) => r.votes >= votesMin)
  }

  if (votesMax !== undefined) {
    reviews = reviews.filter((r) => r.votes <= votesMax)
  }
  ...
  let images = await db<Image>('images').whereNotNull('reviewID')

  const votes = await db<Vote>('votes').whereNotNull('reviewID')

  reviews = reviews.map((r) => {
    const voteSum = votes.filter((v) => v.reviewID === r.reviewID)
      .length

    const upVoteSum = votes.filter(
      (v) => v.reviewID === r.reviewID && v.vote
    ).length

    const voted = votes.find(
      (v) =>
        v.reviewID === r.reviewID && v.userID === req.session?.userID
    )
  ...
  const batch: any = await Promise.all(
    _batch.map(async (r) => {
  ...
  return {
    ...
    images:
      groupID !== undefined
        ? images.filter((i) =>
            reviewIDs.includes(i.reviewID as number)
          )
        : undefined
  }
}
...
const getReviewByID = async (
  ...
  const userHasPermission =
    ...
    (review !== undefined && review.userID === req.session?.userID)

  if (
    review === undefined ||
    (review.moderationStatus !== 'APPROVED' && !userHasPermission)
  )
    throw new StatusError(404, 'Not Found')

  const images = await db<Image>('images').where('reviewID', reviewID)

  const votes = await db<Vote>('votes').where('reviewID', reviewID)
...
const checkExistingReview = async (
  req: Request
): Promise<{ groupID: number; isReviewed: boolean }> => {
...
const uploadReviewImages = async (
  ...
  const uploadedImages: Image[] = await db('images').insert(
    filesWithIndexes,
    ['*']
  )
