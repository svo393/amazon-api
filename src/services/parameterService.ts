...
const addParameter = async (
  ...
  if (addedParameter === undefined) {
    throw new StatusError(
      409,
      `Parameter with name "${parameterInput.name}" already exists`
    )
  }
  return addedParameter
}

const addProductParameters = async (
  productParameterInput: ProductParametersInput,
  req: Request
): Promise<ProductParameter[]> => {
  const {
    rows: addedProductParameters
  }: { rows: ProductParameter[] } = await db.raw(
    `
    ? ON CONFLICT
      DO NOTHING
      RETURNING *;
    `,
    [
      await db('productParameters').insert(
        productParameterInput.map((pp) => ({
          ...pp,
  ...
  if (addedProductParameters === undefined) {
    throw new StatusError(
      409,
      'At least on of parameters is already added to the product'
    )
  }
...
const getParametersByProduct = async (
  req: Request
): Promise<(ProductParameter & Parameter)[]> => {
