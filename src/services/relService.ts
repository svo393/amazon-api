...
const getSimilar = async (
  req: Request
): Promise<ProductRelData | Record<string, unknown>> => {
  const product: Pick<Product, 'title' | 'price' | 'productID'> & {
    stars: string
    reviewCount: string
  } = await getRelsQuery
    .clone()
...
const getAfter = async (req: Request): Promise<ProductRelData[]> => {
  const products: (Pick<Product, 'title' | 'price' | 'productID'> & {
    stars: string
    reviewCount: string
  })[] = await getRelsQuery
    .clone()
...
const getMoreToExplore = async (
  req: Request
): Promise<ProductRelData[]> => {
...
const getCompare = async (
  req: Request
): Promise<ProductRelData[] | null> => {
  const { productID } = req.params

  const products: (Pick<
    Product,
    'title' | 'price' | 'productID' | 'groupID'
  > & { stars: string; reviewCount: string })[] = await getRelsQuery
    .clone()
  ...
  const parameters = productParameters
    .filter(
      (pp) =>
        curProductParameterIDs.includes(pp.parameterID) &&
        pp.productID !== Number(productID)
    )
    .reduce((acc, cur) => {
  ...
  > & {
    stars: string
    reviewCount: string
    ratingStats: { stars: number; count: string }[]
  })[] = await Promise.all(
    batchWithCounts.map(async (p) => ({
      ...p,
      ratingStats: (await db('reviews')
        ...
        .groupBy('stars')) as { stars: number; count: string }[]
    }))
  )
  ...
  return _batch.map((p) => ({
    ...
    stars: parseFloat(p.stars),
    reviewCount: parseFloat(p.reviewCount),
    productParameters: flatten(_parameters).filter(
      (pp: any) => pp.productID === p.productID
    ),
