...
router.post('/', requireAdmin, async (req, res) => {
  ...
  const addedShippingMethod = await shippingMethodService.addShippingMethod(
...
router.get('/:shippingMethodName', async (req, res) => {
  const shippingMethod = await shippingMethodService.getShippingMethodByName(
    req
  )
...
router.put('/:shippingMethodName', requireAdmin, async (req, res) => {
  ...
  const updatedItem = await shippingMethodService.updateShippingMethod(
